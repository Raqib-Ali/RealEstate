    ...
    return <div>
...

        {
            listing && !error && !loading && <div className="mt-2">
                ...
                >
                    {
                        listing?.imgUrls.map(images => <SwiperSlide><img className="w-full h-60 sm:h-96" src={images} alt="" /></SwiperSlide>)
                    }
                ...
                <div className="max-w-3xl mx-auto flex flex-col gap-4 mt-4 p-2">
                    ...
                    {/* Title */}
                    <h1 className="text-2xl font-semibold">{listing.name} - {(listing.type == 'rent')?<span>${listing.regularPrice}/month</span>:<span>${listing.regularPrice}</span>}</h1>
                    ...
                    <div className="flex gap-4">
                        <button className="bg-red-800 px-8 py-1 text-white rounded-lg">{ listing.type == 'rent'? 'For Rent':'For Sale'}</button>
                    ...
                    <p><span className="font-semibold">Description - </span> {listing.description}</p>
                     ...
                     {/* Perks */}
                    <ul className="flex gap-3 sm:gap-8 flex-wrap text-green-800 font-semibold">
